        ...
        for (index, node) in enumerate(self.nodes):
            ...
            for offset in range(-neighbour_range, neighbour_range + 1):
                if offset != 0:  # Skip connecting a node to itself
                    neighbour_index = (
                                                  index + offset) % N  # Calculates the index of the neighbouring node in the network
                    node.connections[
                        neighbour_index] = 1  # This creates a connection between the current node and its neighbour
                    self.nodes[neighbour_index].connections[
                        index] = 1  # This creates a connection between the nieghbour and the current index
        ...
        for node_index in range(len(self.nodes)):
            ...
            for connection_ind in connection_inds:  # This iterates over each connection
                if np.random.random() < re_wire_prob:  # Implements the re-wire probability  by choosing a random number between 0 and 1, if this number is less than prob, carry out behaviour
                    ...
                    node.connections[
                        connection_ind] = 0  # sets the current connection between the index and neighbour to 0
                    self.nodes[connection_ind].connections[
                        node_index] = 0  # sets the current connection between the nighbour and index to 0
